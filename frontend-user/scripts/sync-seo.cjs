...
function buildPlotTitle(plot) {
  ...
  if (title && location) return `${title} | ${location} | tstplotconnect`;
  if (title) return `${title} | tstplotconnect`;
  const fallback = [stripText(plot.category || "Listing"), stripText(plot.county || plot.town || plot.country || "Kenya")].filter(Boolean).join(" in ");
  return `${fallback || "Property Listing"} | tstplotconnect`;
}
...
function buildPlotDescription(plot) {
  ...
  const pricePart = Number.isFinite(Number(plot.price))
    ...
    : "";
  const details = stripText(plot.description) || "View photos, location details, and open the live listing on tstplotconnect.";
  return truncate(`View ${title}${place ? ` in ${place}` : ""}${pricePart} on tstplotconnect. ${details}`, 158);
}
  ...
  <main>
    <div class="hero">
      <div>
        ...
        <h1>${escapeHtml(stripText(plot.title) || "Property listing on tstplotconnect")}</h1>
      ...
      <div class="card">
        ...
        <p>${escapeHtml(stripText(plot.description) || "This verified property listing is available on tstplotconnect. Open the live listing to view current search filters, nearby options, and map context.")}</p>
      </div>
    </div>
  </main>
</body>
...
async function main() {
  ...
  const hadLegacyListings = fs.existsSync(LISTINGS_DIR);
  if (hadLegacyListings) {
    fs.rmSync(LISTINGS_DIR, { recursive: true, force: true });
  }
  ...
  console.log(`Generated ${path.relative(FRONTEND_DIR, ROBOTS_PATH)}.`);
  if (hadLegacyListings) {
    console.log("Removed legacy property listing SEO pages from public/listings.");
  }
