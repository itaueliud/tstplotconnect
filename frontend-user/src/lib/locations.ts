...
export function countyMetaDescription(country: string, county: string, towns: string[] = []): string {
  if (towns.length === 0) {
    return `Find verified plots, hostels, bedsitters, and lodges in ${county}, ${country}. Open filtered listings directly on tstplotconnect.`;
  }

  const preview = towns.slice(0, 4).join(", ");
  const suffix = towns.length > 4 ? ` and ${towns.length - 4} more towns` : "";
  return `Find verified plots, hostels, bedsitters, and lodges in ${county}, ${country}, including ${preview}${suffix}. Open filtered listings directly on tstplotconnect.`;
}
...
export function countySeoKeywords(country: string, county: string, towns: string[] = []): string[] {
  ...
  return Array.from(new Set([
    ...
    `tstplotconnect ${countyLower}`,
    `tstplotconnect ${countryLower}`,
    ...townKeywords
  ]));
}
