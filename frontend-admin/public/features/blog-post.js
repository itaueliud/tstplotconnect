const DEFAULT_API_BASE = "https://tstplotconnect-tvn3.onrender.com";
...
function setMeta(title, description, slug) {
  document.title = `${title} | tstplotconnect`;
...
function setSchema(post) {
  ...
  const schema = {
    ...
    author: {
      "@type": "Person",
      name: post.author || "tstplotconnect"
    },
    publisher: {
      "@type": "Organization",
      name: "tstplotconnect",
...
function renderRelated(posts, currentSlug) {
  ...
  filtered.forEach((post) => {
    ...
    excerpt.textContent = post.excerpt || "More rental guidance from tstplotconnect.";
    ...
    meta.textContent = post.author || "tstplotconnect";
